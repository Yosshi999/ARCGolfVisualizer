def p(g):
 ...
 H=[(i,j)for i in range(30)for j in range(29)if B!=g[i][j]!=C==g[i][j+1]]
 V,I,J=max((sum([C!=g[i+u][j+v]!=B for u in range(-2,3)for v in range(-2,3)if abs(u)+abs(v)<=2]),i,j)for i in range(2,28)for j in range(2,28)if B!=g[i][j]!=C)
 for i,j in H:
  for u in [-1,0,1]:
   for v in [-1,0,1]:
