...
def p(g):
 ...
 for i in range(-H,H*2):
  for j in range(-W,W*2):
   for F in lambda y,x:(y+i,x+j),lambda y,x:(y+i,j-x),lambda y,x:(i-y,x+j),lambda y,x:(i-y,j-x),lambda y,x:(x+j,y+i),lambda y,x:(j-x,y+i),lambda y,x:(x+j,i-y),lambda y,x:(j-x,i-y):
