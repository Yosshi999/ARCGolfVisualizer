...
def q(g):
 for i in R(len(g)-2):
  for j in R(len(g[0])-2):
   if len(set(g[i+1][j+1:j+3]+g[i+2][j+1:j+3]))==4and sum(g[i][j:j+2]+g[i+1][j:j+2])>(c:=g[i+1][j+1]):
    for u,v in D([*map(list,g)],i+1,j+1,c):g[2*i+3-u][v]|=g[i+2][j+1];g[u][2*j+3-v]|=g[i+1][j+2]
