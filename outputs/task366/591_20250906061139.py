...
def p(g):
 ...
 U=M(S(g));G=S(T);L=len(G);B=M(G);D=lambda z:L>z>=0<(c:=G[z])!=B and sum((D(z+a)for a in[-1,-W,-~z%W>0,W][z%W<1:G.__setitem__(z,B)]),[(z,c)])or[];s=[*map(D,r(L))];G=S(g);C=M([a[1]for a in S(s)])
