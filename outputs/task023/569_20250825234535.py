def q(g,u,G):
 I=lambda i:0<=i<len(g);J=lambda j:0<=j<len(g[0]);D=lambda u:(u//len(g[0]),u%len(g[0]));h=[*map(list,g)];i,j=D(u);B=lambda i,j,Y,X:all(I(i+y)and J(j+x)and h[i+y][j+x]==5for y,x in zip(Y,X))
