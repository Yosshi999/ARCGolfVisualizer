...
def validate_code(src: str, sample_case: dict, timeout_sec: float = 0.5) -> bool:
 """
...
def run_optimizer(raw_function_string: str, sample_case: dict, pruning_threshold: int = 500) -> str:
 """
 ...
 for i in range(LIMIT):
  if i % 500 == 0:
   print(f"[SA] Iter {i}, T={T:.3f}, best={best_size}, current={current_size}")
  ...
  if delta <= 0 or random.random() < math.exp(-delta / T):
   ...
   if trial_size < best_size:
    ...
    print(f"[SA] New best at iter {i+1}: size={best_size}")
