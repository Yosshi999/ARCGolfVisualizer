# https://github.com/python/cpython/blob/3.14/Lib/_ast_unparse.py
import sys
from ast import *
...
class Unparser(NodeVisitor):
    ...
    def fill_terminal(self, term: Terminal, *, allow_semicolon=True):
        if self._simple_stmts:
            assert allow_semicolon
            if self._source and self._source[-1].value != ":":
                self.maybe_semicolon()
            self.write_terminal(term)
        elif self._prev_indent == self._indent and allow_semicolon:
            self.maybe_semicolon()
            self.write_terminal(term)
        else:
            self.maybe_newline()
            self.write_terminal(RESERVED(" " * self._indent))
            self.write_terminal(term)
        self._prev_indent = self._indent

    def fill(self, text="", *, allow_semicolon=True):
        self.fill_terminal(RESERVED(text), allow_semicolon=allow_semicolon)

    def write(self, text):
        if text:
            self._source.append(RESERVED(text))
    
    def write_terminal(self, term: Terminal):
        if term.value:
            self._source.append(term)
    ...
    def visit(self, node) -> List[Terminal]:
...
def unparse(tree):
    res = Unparser().visit(tree)
    # space optimization
    if not res:
        return ""
    string = res[0].value
    for i in range(len(res)-1):
        if confusion_check(res[i], res[i+1]):
            string += " "
        string += res[i+1].value
    return string